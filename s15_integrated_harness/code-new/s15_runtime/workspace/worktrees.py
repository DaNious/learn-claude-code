from ..foundation.bootstrap import WORKDIR, Path, re, subprocess
from .tasks import (
    Task,
    TeammateAssignment,
    _owner_in_progress,
...
def assignment_cwd(owner: str) -> Path:
    with task_lock:
        ...
        if task and (not assignment or assignment.get("task_id") != task.id):
            ...
            assignment = TeammateAssignment(task_id=task.id, cwd=cwd)
...
def release_completed_assignment(owner: str) -> bool:
    ...
    with task_lock:
        ...
        if owner in messaging.plan_gates:
            messaging.plan_gates[owner] = "not_required"
        return True
...
def remove_worktree(name: str, discard_changes: bool = False) -> str:
    ...
    with task_lock:
        ...
        with background.background_lock:
            running = [task for task in background.background_tasks.values()
                       if task.get("status") == "running"
                       and task.get("cwd")
                       and Path(cast(str, task["cwd"])).resolve() == path.resolve()]
        if running:
