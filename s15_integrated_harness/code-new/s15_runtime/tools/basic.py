...
from ..workspace.worktrees import assignment_cwd
...
def _stop_process_group(process: subprocess.Popen):
    ...
    for sig in (signal.SIGTERM, signal.SIGKILL):  # pyright: ignore[reportAttributeAccessIssue]
        try:
            os.killpg(process.pid, sig)  # pyright: ignore[reportAttributeAccessIssue]
