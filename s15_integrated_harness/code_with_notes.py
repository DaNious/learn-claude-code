...
# 补充说明：自定义 API 地址通常使用 API Key 鉴权；这里主动移除
# ANTHROPIC_AUTH_TOKEN，避免它比预期的 Key 配置优先而导致鉴权混乱。
...
def load_memory_runtime():
    ...
    # 补充说明：这里不是普通 import。动态加载会创建一份独立的 s09 模块实例，
    # 随后再把本运行时的工作目录、客户端和模型注入进去，实现“复用实现、共享状态”。
...
class ConsoleBroker:
    ...
    def ask(self, prompt: str) -> str:
        # 补充说明：主 CLI 和权限确认都可能读 stdin；锁保证同一时刻只有一个提问，
        # 否则两个线程的提示与用户输入可能互相串线。
        with self._lock:
...
def terminal_print(text: str):
    # 补充说明：后台线程打印事件时，先清除当前输入行，输出事件后再恢复提示符和
    # readline 中尚未提交的文本，从而尽量不破坏用户正在键入的命令。
...
def task_store_lock():
    ...
    # 补充说明：RLock 只协调当前 Python 进程内的线程，flock 协调多个宿主进程；
    # thread-local depth 让 save_task 等嵌套调用复用最外层文件锁，避免重复开锁。
    with task_lock:
...
def advance_assignment_version(owner: str):
    ...
    # 补充说明：version 相当于 assignment 的“代数”。任务切换后旧计划即使迟到，
    # 也会因为代数不匹配而失效，防止旧批准被套用到新任务上。
    with task_lock:
...
def update_task(task_id: str, addBlockedBy: list[str]) -> Task:
    ...
    with task_store_lock():
        ...
        for dependency in dependencies:
            ...
            # 补充说明：准备新增 task -> dependency 时，要反向检查 dependency
            # 是否已经（直接或间接）依赖 task；若是，新增边就会闭合成环。
...
def save_task(task: Task):
    with task_store_lock():
        ...
        try:
            ...
            # 补充说明：先写同目录临时文件再 replace，可避免进程在写到一半时
            # 留下截断 JSON；同一文件系统内 replace 也提供原子替换语义。
...
def claim_task(task_id: str, owner: str = "agent") -> str:
    ...
    with task_store_lock():
        ...
        save_task(task)
        # 补充说明：持久化 Task 记录“谁拥有任务”，内存中的 assignment 还充当
        # cwd 租约；后续所有文件工具都会通过它固定到同一个工作目录。
...
def complete_task(task_id: str, owner: str = "agent") -> str:
    ...
    with task_store_lock():
        ...
        if not assignment or assignment.get("task_id") != task.id:
            ...
            teammate_assignments[owner] = {"task_id": task.id, "cwd": cwd}
        # 补充说明：这里只完成持久化任务，不立刻释放 cwd 租约。租约要等模型回合
        # 边界再释放，避免同一批尚未执行完的工具调用突然切换目录。
...
def _registered_worktree(name: str) -> tuple[Path | None, str | None]:
    # 补充说明：目录存在并不代表它仍是可信 worktree；还要同时核对 Git 注册表、
    # 实际目录以及约定的 wt/<name> 分支，任何不一致都按失败处理。
    try:
...
def assignment_cwd(owner: str) -> Path:
    with task_lock:
        ...
        # 补充说明：Task 在磁盘上持久化，assignment 只在内存中。进程恢复或映射
        # 丢失时，可从 owner 的 in_progress Task 重建租约，但仍需重新验证 worktree。
...
def create_worktree(name: str, task_id: str) -> str:
    ...
    with task_lock:
        ...
        # 补充说明：Git 创建与 Task JSON 绑定是两个独立步骤，无法组成真正事务。
        # 因此失败路径刻意保留 Git 产物并报告“部分成功”，不自动删除潜在工作。
...
def remove_worktree(name: str, discard_changes: bool = False) -> str:
    ...
    with task_lock:
        ...
        # 补充说明：这里只移除 checkout，不删除 wt/<name> 分支；即使显式丢弃
        # 工作区改动，已提交到分支的历史仍会保留，便于恢复。
...
def _parse_frontmatter(text: str) -> tuple[dict, str]:
    # 补充说明：只有首行和后续闭合分隔符都存在时才把 YAML 当作元数据；
    # YAML 无效或结果不是映射时退化为空元数据，技能正文仍可继续使用。
...
def assemble_system_prompt(context: dict) -> str:
    ...
    # 补充说明：该提示每个模型回合都会重建，所以新连接的 MCP、记忆召回结果和
    # 队友状态无需手工同步到旧字符串，下次调用自然可见。
...
def safe_path(path: str, cwd: Path | None = None) -> Path:
    # 补充说明：这里的安全边界是当前工具的 base。绑定 worktree 后 base 会变成
    # worktree 根目录，因此相对路径不能用“..”逃逸到主仓库或其父目录。
...
def _run_bash_process(command: str, cwd: Path | None = None) -> tuple[str, int | None]:
    process = None
    try:
        # 补充说明：start_new_session 令命令成为新进程组组长；超时或宿主退出时，
        # killpg 才能连同命令派生出的子进程一起清理，而不只终止外层 shell。
...
def _agent_cwd() -> tuple[Path | None, str | None]:
    # 补充说明：主 agent 也走与队友相同的 assignment_cwd 入口；下方包装器由此
    # 统一把 bash/read/write/edit/glob 路由到任务绑定的目录。
    try:
...
class MessageBus:
    ...
    def _read_unlocked(self, agent: str) -> list[dict]:
        ...
        msgs = [json.loads(line) for line in inbox.read_text().splitlines()
                if line.strip()]
        # 补充说明：邮箱采用“读后删除”的 JSONL 队列，返回的这批消息只消费一次。
        # Condition 负责唤醒本进程线程，磁盘文件则让消息状态可观察、可跨线程共享。
        inbox.unlink()
        return msgs
...
def match_response(response_type: str, request_id: str, approve: bool,
    ...
    with team_lock:
        ...
        if not state:
            ...
            return False
        # 补充说明：request_id 之外还校验响应类型、发送者和接收者，防止一个合法
        # ID 被错误类型或错误参与者的消息“认领”。
        expected = {
...
def current_work_identity(owner: str) -> tuple[int, str | None]:
    # 补充说明：计划审批同时绑定 assignment version 与 task_id；二者共同构成
    # 当前工作的身份，用于识别切换任务后才到达的过期审批。
    with task_lock:
        assignment = teammate_assignments.get(owner)
...
def _run_teammate_tool(name: str, block, handlers: dict) -> str:
    ...
    # 补充说明：计划门禁只拦截会执行命令或改文件的工具；读文件、列任务和提交
    # 计划仍可用，否则队友无法收集信息并完成审批流程。
...
def spawn_teammate_thread(name: str, role: str, prompt: str,
    ...
    def run_loop():
        # 补充说明：队友线程是一个小型 agent 状态机：处理邮箱 -> 调模型 ->
        # 执行工具 -> 上报最终文本 -> 空闲等待消息或自动领取下一个就绪任务。
        ...
        while not should_stop:
            ...
            gate = plan_gates.get(name, "not_required")
            # 补充说明：提交计划后模型可能输出一段“最终文本”，但 pending 时不能
            # 把它当任务结果上报；线程转入 waiting_approval，等待 Lead 的响应。
            ...
            while True:
                # 补充说明：wait_for_messages 使用 Condition 阻塞；每次超时才扫描
                # 任务板，因此既能快速响应消息，也无需持续忙轮询任务文件。
...
def _teammate_submit_plan(from_name: str, plan: str) -> str:
    with task_lock:
        ...
        with team_lock:
            ...
            # 补充说明：在请求中快照当前工作身份。审批返回时即使 request_id 正确，
            # 只要任务或 assignment version 已变化，也不会打开新任务的执行门禁。
...
def trigger_hooks(event: str, *args):
    # 补充说明：第一个返回非 None 的 hook 会短路后续 hook。对 PreToolUse 而言
    # 这就是拒绝原因；全部返回 None 才表示允许继续分发工具。
...
def permission_hook(block):
    ...
    if block.name == "bash":
        ...
        # 补充说明：后台/异步线程无法安全地与主 CLI 争抢交互式输入，因此需要
        # 人工确认的操作在这些线程中直接拒绝，而不是弹出确认。
...
def spawn_subagent(description: str) -> str:
    # 补充说明：这个 subagent 是同步、一次性的局部循环，不加入 MessageBus，
    # 也没有持久队友的任务租约与自动领取机制；调用者会一直等到它给出摘要。
...
def unseen_tool_result_positions(messages: list) -> set[tuple[int, int]]:
    ...
    # 补充说明：最近一次 assistant 消息之后的 tool_result 尚未被下一次模型调用
    # 消费，micro_compact 必须完整保留；更早的结果才可以替换成占位说明。
    last_assistant = next(
...
def persist_large_output(tool_use_id: str, output: str) -> str:
    ...
    # 补充说明：完整输出落盘后，上下文中只保留路径与预览；这减少 token 占用，
    # 同时允许 agent 之后通过 read_file 按需取回原始内容。
...
def snip_compact(messages: list, max_messages: int = 50) -> list:
    ...
    # 补充说明：裁剪边界若正落在 assistant tool_use 与其 user tool_result 之间，
    # 就移动边界以保留完整配对，避免生成 API 不接受的孤立工具消息。
...
def micro_compact(messages: list) -> list:
    ...
    # 补充说明：只压缩模型已经看过的结果，并保留最近 KEEP_RECENT_TOOL_RESULTS
    # 个结果原文，在节省上下文与保持短期可追溯性之间折中。
...
def compact_history(messages: list, active_request: str) -> list:
    ...
    # 补充说明：摘要被明确标成不可信参考，当前请求单独作为 authoritative 字段。
    # 这是为防止旧工具输出或旧对话中的指令在压缩后被误当成新的授权。
...
def with_retry(fn, state: RecoveryState):
    # 补充说明：state 跨 agent_loop 的多次模型调用复用。429 仅退避重试；连续
    # 529 达阈值时可切备用模型，并把该选择带到本次 agent 回合的后续调用。
...
def start_background_task(block, handlers: dict) -> str:
    ...
    # 补充说明：在线程启动前快照 cwd，避免后台命令运行期间主 agent 完成任务、
    # 释放 assignment 后，命令的工作目录随全局状态漂移。
    ...
    # 补充说明：worker 闭包虽引用稍后定义的 bg_id，但线程在 bg_id 创建后才启动，
    # 因而正常路径上不会读取到未绑定变量。
...
def collect_background_results() -> list[str]:
    # 补充说明：完成项在持锁状态下从两个字典中 pop，通知因此具备“至多投递一次”
    # 的进程内语义；收集后再次调用不会重复注入相同结果。
    with background_lock:
...
def cron_matches(cron_expr: str, dt: datetime) -> bool:
    ...
    if not (m and h and month_ok):
        return False
    # 补充说明：沿用传统 cron 语义：日期与星期都被具体限制时二者取 OR；其中
    # 任一为 * 时只检查另一个，而不是始终要求两者同时满足。
    if dom == "*" and dow == "*":
        return True
    if dom == "*":
        return dow_ok
...
def _enqueue_due_job(job: CronJob):
    ...
    # 补充说明：一次性持久任务先把 pending_delivery 落盘，再进入内存队列。
    # 若进程在模型接收前崩溃，重启时 load_durable_jobs 会重新排队，避免静默丢失。
...
def cron_scheduler_loop():
    while True:
        time.sleep(1)
        now = datetime.now()
        # 补充说明：调度线程每秒扫描一次，而 cron 精度只有分钟；marker 用来确保
        # 同一 job 在同一分钟内只入队一次。
...
def acknowledge_cron_jobs(jobs: list[CronJob]):
    ...
    # 补充说明：只有模型调用成功返回后才确认投递并删除一次性任务；模型调用失败
    # 则由 restore_cron_jobs 放回队列，形成近似“至少投递一次”的保障。
...
def assemble_tool_pool() -> tuple[list[dict], dict]:
    ...
    for server_name, mcp_client in mcp_clients.items():
        ...
        for tool_def in mcp_client.tools:
            ...
            # 补充说明：lambda 默认参数在创建时绑定当前 client/tool，避免 Python
            # 闭包晚绑定导致所有 MCP handler 最终都调用循环中的最后一个工具。
            handlers[prefixed] = (
...
def update_context(context: dict, messages: list) -> dict:
    # 补充说明：参数 context 没有被增量修改；函数每次都从真实运行时状态重建
    # 快照，避免已断开的 MCP 或已退出队友残留在旧上下文中。
    return {
...
rounds_since_todo = 0
# 补充说明：交互式主线程和自动事件线程共享 history/context；agent_lock 保证
# 任一时刻只有一个 agent_loop 修改对话，防止两次模型回合交错写入。
...
def agent_loop(messages: list, context: dict, active_request: str):
    ...
    while True:
        ...
        # 补充说明：到这里说明模型已经接收这批 scheduled prompt；此时才确认 cron，
        # 早于此处的异常会走 restore，防止一次性任务因临时 API 故障丢失。
        acknowledge_cron_jobs(unacknowledged_cron_jobs)
        unacknowledged_cron_jobs.clear()

        if response.stop_reason == "max_tokens":
            if not state.has_escalated:
                # 补充说明：第一次截断响应不写入 history，而是提高输出预算重做同一
                # 次调用；第二次仍截断才保留响应并追加 continuation prompt。
        ...
        for block in response.content:
            ...
            if block.name == "compact":
                # 补充说明：compact 是循环识别的伪工具，没有普通 handler。先生成
                # 对应 tool_result 保持消息协议完整，再在本轮工具处理后整体压缩。
                results.append({
                    "type": "tool_result",
                    "tool_use_id": block.id,
...
def async_event_loop(history: list, context: dict, session_state: dict):
    while True:
        time.sleep(1)
        with agent_lock:
            # 补充说明：这里只窥视 cron_queue 来判断是否需要唤醒；真正消费由随后
            # 的 agent_loop 完成，因而不会在移交前把任务从队列中取走。
            with cron_lock:
                fired = list(cron_queue)
...
if __name__ == "__main__":
    ...
    # 补充说明：后台事件线程和下面的交互循环共用同一 history/context；两边进入
    # agent_loop 前都获取 agent_lock，因此定时/团队事件不会与用户请求并发改历史。
